...
import { CourseRequestModal, CourseRequestsPanel } from "@/app/components/CourseRequests";
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 12, color: T.text2, margin: 0 }}>
        {course.faculty ? `${course.faculty} · ` : ""}{course.department}
        {course.year_level ? ` · Year ${course.year_level}` : ""}
      </p>
...
export default function CoursesPage() {
  ...
  useEffect(() => {
    ...
    // Deep links from notifications: /courses?tab=requests or ?tab=browse
    const wanted = new URLSearchParams(window.location.search).get("tab");
    if (wanted === "requests" || wanted === "browse" || wanted === "my") setTab(wanted);
  }, [router]);
  ...
  const filteredAll = useMemo(() => {
    ...
    return allCourses.filter((c) => {
      ...
      const matchesYear = yearFilter === "all" || String(c.year_level ?? "") === yearFilter;
      return matchesSearch && matchesDept && matchesYear;
    });
  }, [allCourses, search, deptFilter, yearFilter]);
      ...
      <main style={{ flex: 1, overflowY: "auto", padding: "28px 32px" }}>
        ...
        <div style={{ display: "flex", gap: 4, borderBottom: `1px solid ${T.border}`, marginBottom: 18 }}>
          {([
            ...
            { id: "requests" as Tab, label: isAdmin ? "Course requests" : "My requests" },
          ]).map((t) => {
        ...
        {tab === "requests" && (
          <>
            ...
            <CourseRequestsPanel isAdmin={isAdmin} onResolved={fetchAllCourses} />
          </>
        )}
        ...
        {tab === "browse" && (
          <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            >
              ...
              {[1, 2, 3, 4, 5].map(y => (
                <option key={y} value={String(y)}>Year {y}{y === 5 ? " (grad)" : ""}</option>
              ))}
            </select>
            <button
              onClick={() => setRequestOpen(true)}
        ...
        {requestOpen && (
          <CourseRequestModal
            ...
            onRequested={(msg, ok) => showToast(msg, ok)}
          />
        )}
      </main>
    </div>
