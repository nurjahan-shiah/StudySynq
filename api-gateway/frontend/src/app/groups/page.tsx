...
function GroupCard({ g }: { g: MyGroup }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 10, fontSize: 11, color: T.text2, marginBottom: 10, flexWrap: "wrap" }}>
        ...
        {(g.session || g.section) && (
          <span style={{
            ...
            {[g.session, g.section ? `Sec ${g.section}` : null].filter(Boolean).join(" · ")}
          </span>
        )}
...
export default function GroupsPage() {
  ...
  async function createGroup() {
    ...
    if (session && !/^(F|W|SU|Y)\d{2}$/i.test(session.trim())) {
      setFormError("Session must look like F25, W26, SU26, or Y25.");
      return;
    }
    setSaving(true);
    setFormError(null);
    const res = await apiClient.post<{ id: string }>("/groups", {
      ...
      session: session.trim().toUpperCase() || null,
        ...
        {creating && (
          <div style={{
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: T.text2, marginBottom: 6 }}>
                  Session <span style={{ fontWeight: 400 }}>(e.g. SU26)</span>
                </label>
                <input
                  value={session}
                  onChange={(e) => { setSession(e.target.value); setFormError(null); }}
              ...
              <div style={{ flex: 1 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: T.text2, marginBottom: 6 }}>
                  Section <span style={{ fontWeight: 400 }}>(e.g. A, B, M)</span>
                </label>
                <input
                  value={section}
